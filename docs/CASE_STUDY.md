...
主要使用者是需要在送件前自行檢查補助申請或收據的人，以及協助家人整理文件的使用者。介面以「沉著、清楚、可信」為方向，像一張整理好的公共服務工作檯：資訊足夠但不壓迫，提示直接但不製造焦慮。

設計時採用以下原則：

- 兩秒內看懂準備文件、確認設定、同意並開始、查看結果四步流程。
- 等待、處理中、錯誤與完成狀態固定靠近主要按鈕；處理中阻止重複送出。
- 空狀態要說明接下來會出現什麼，不留下看似故障的空表格。
- 結果先回答「下一步怎麼做」，技術欄位、完整檢核與 JSON 放在次要分頁。
- 唯讀結果使用語意表格與局部捲動，不顯示不存在的匯入、排序或編輯操作。
- 以 WCAG 2.2 AA 為基準，正文維持清楚字級與對比、互動目標至少 44×44px，狀態不只依靠色彩表達。
...
### 2b. 把「模型宣稱」和「已驗證來源」分成兩層

v1.0 保留了頁碼與證據，但沒有回答一個更根本的問題：**那兩個值也是模型生成的**。使用者看到「第 2 頁」時，沒有任何東西能證明那段文字真的在第 2 頁。

v1.1 因此加入一層確定性的後處理：抽取完成後，系統在文件自己的 PDF 文字層裡搜尋模型引用的原文，找到唯一一處才輸出 bounding box。模型**不被允許**輸出座標——如果讓它輸出，它會生成一個看起來很合理但沒有根據的框，那正是這一輪要消除的失敗模式。

最關鍵的取捨是**重複即模糊**：只要同一段證據在文件中出現超過一次，即使其中一處落在模型宣稱的頁上，系統也回報 `ambiguous` 而不給位置。用宣稱頁去消歧義，在「頁碼錯誤 + 值重複」同時發生時會產生一個看起來已驗證、實際指錯地方的框。寧可少蓋幾個框，也不能有一個假的綠燈。這個選擇讓實際覆蓋率停在 85%，而 false verified rate 是 0%。
...
獨立於決策層，另有一套來源核驗評估：4 份合成 PDF、61 個欄位（51 個有 ground-truth 頁碼與 bbox），涵蓋重複值、錯誤頁碼、幻覺證據、換行接合、旋轉頁、算繪後縮放、巢狀清單與純影像頁。Ground truth 由文件生成器的版面規格與字型量測直接記錄，不從系統輸出反推。
...
方法、威脅模型、比對政策與已知限制見 [EVIDENCE_PROVENANCE.md](EVIDENCE_PROVENANCE.md)。合成語料的 IoU 接近 1.0 反映的是語料性質（ground truth 與 resolver 同樣以字型度量為基礎），驗證的是**政策**而非真實版面雜訊下的定位誤差。
...
Playwright 離線 fixture 會以 1920×1080、1440×1000 與 390×844 三種尺寸檢查 responsive layout、鍵盤焦點、互動目標、空狀態、四種合成結果與 light／dark 系統偏好。最近一次驗收沒有未命名或過小的互動控制、手機水平溢出、console error 或 page error；七個關鍵文字區塊的最低對比為 6.89:1，高於 WCAG AA 正文門檻 4.5:1。

稽核報告與 wide／mobile／結果截圖屬可重建的測試產物，執行時寫入被 Git 忽略的 `outputs/ui-audit/`；公開倉庫只保留 README 使用的 desktop 預覽，避免把一次性驗收證據長期堆在 source tree。
...
- 來源核驗只涵蓋有原生文字層的頁面；掃描件在預設安裝下維持 `page_only`，可選 OCR 路徑尚無量化評估集。
...
1. 為可選的本機 OCR 定位路徑建立同一套 gate 的量化評估集。
2. 建立匿名真實版面 error taxonomy，分離 OCR、欄位對齊、型別與規則錯誤。
3. 在相同評估集比較地端模型與雲端基準。
4. 增加人工確認回饋，但只保存使用者明確同意的去識別結果。
5. 依公開使用情況再評估持久化 rate limit 與排程佇列。
