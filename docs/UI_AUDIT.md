...
> 日期：2026-07-23
> 範圍：本機 Gradio 介面、補助綠／黃／紅與收據綠結果、1920×1080、1440×1000、390×844
> 方法：Playwright 離線 fixture、DOM／鍵盤／尺寸量測與 CSS 檢查；未呼叫模型 API
...
| 4 | Theming | 2/4 | 2/4 | 核心 token 完整，仍有硬編碼的狀態與表面色待逐步收斂 |
| 5 | Anti-Patterns | 3/4 | 4/4 | 移除 Dataframe 虛假操作與結果分頁的大模糊陰影 |
| **Total** |  | **15/20** | **18/20** | **Excellent — 核心可用性問題已修正** |
...
- 主要色彩對比：正文 5.73–5.85:1、主要按鈕 7.69:1、紅燈 6.88:1、黃燈 6.45:1，皆達 WCAG AA。
- 唯讀表格內容在輸出前經 HTML escaping；`<script>` 與 `<img onerror>` 測試字串只會顯示成文字。
- 所有離線案例 console error 0、page error 0。

## Verification

- `uv run pytest tests/test_ui.py tests/test_ui_fixture.py -q` → 19 passed。
- `uv run --all-extras pytest --cov=doc_inspector --cov-report=term-missing -q` → 116 passed、coverage 89%。
- `uv lock --check` → 通過。
...
- 完整機器可讀報告：`docs/assets/ui-quality-audit.json`。
